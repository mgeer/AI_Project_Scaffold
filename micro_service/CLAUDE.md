...
- 禁止全量扫描项目；按 DDD 层级定位文件，找到即停
- 回复中禁止大段复述代码，用 `文件路径:行号` 引用
- 每次任务只处理一个模块，跨模块影响须拆成独立任务
...
- 业务规则（如"金额不能为负"、"状态机流转"）→ Domain Entity 或 Domain Service
- 跨多个 Aggregate 的协调逻辑 → Application Use Case；业务规则本身仍定义在 Domain 对象内
- Repository 接口 → 定义在 Domain 层；实现 → Infrastructure 层
- 有唯一标识且有生命周期 → Entity；无标识且不可变 → Value Object
- 跨 Entity 的业务逻辑且不属于任何单一 Entity → Domain Service
...
- 异常跨层转换：Domain 抛 DomainException（只表达业务含义）；Application 按需包装，禁止透传；Interface 统一映射 HTTP 状态码
...
- 禁止使用 Mock 框架（Mockito / unittest.mock 等）；统一使用 In-memory Fake 替代，测可观测结果而非调用细节
- 每个 Repository / 外部服务接口须提供对应的 In-memory Fake 实现，与生产实现同级维护

## 4. 环境隔离约束

- 所有环境差异通过环境变量注入，禁止硬编码 API Key / 密码等密钥，禁止出现 `if env == "production"` 类逻辑
- `.env` 文件必须加入 `.gitignore`；lockfile 必须纳入版本控制
- 服务启动时通过统一配置验证模块校验所有必要环境变量，缺失立即 Fail Fast
- 服务必须支持优雅关闭（Graceful Shutdown）：处理完当前请求再退出

## 5. 代码质量约束

基于 Clean Code + SOLID（S/O/I）原则：

- 函数职责单一（SRP），行数 ≤ 20；圈复杂度 ≤ 10
- 新增功能通过扩展实现，不修改已稳定接口（OCP）
- 接口小而专注，禁止设计胖接口（ISP）
- 禁止魔法数字/字符串（必须命名为常量）、单字母变量、`data` / `tmp` / `util` 等模糊命名
- 注释解释"为什么"，不解释"是什么"；禁止提交注释掉的代码块
- 相同逻辑出现两次以上必须抽象复用（DRY）

## 6. 可观测性约束

- 日志使用结构化格式（JSON），每条必须含 `trace_id`；`trace_id` 从 `X-Request-ID` 请求头读取，调用下游时透传
- 禁止裸 `print` / `console.log`，统一使用日志库；禁止静默 catch
- 日志级别：ERROR（需人工介入）/ WARN（潜在问题）/ INFO（关键业务事件）/ DEBUG（生产环境禁用）
- 日志禁止包含密钥、密码、PII；关键业务路径必须有 metrics 埋点；服务必须暴露 `/health` 端点
